...
#' @keywords internal
gd_theta2 <- function(y,x,w,beta,alpha1,alpha2,phi,beta_t,alpha1_t,alpha2_t){
  P <- length(beta)
  f <- function(theta){
...
#' @keywords internal
glasso2 <- function(beta,alpha1,alpha2,phi,vlambda1,vlambda2){
  P <- length(beta)
  beta_t <- rep(0,P)
  alpha1_t <- rep(0,P)
  alpha2_t <- rep(0,P)
...
#' @keywords internal
gd_phi2 <- function(phi_0,beta,alpha1,alpha2,beta_t,alpha1_t,alpha2_t){
  phi <- phi_0
  P <- length(beta)
