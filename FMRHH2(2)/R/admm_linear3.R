#' ADMM algorithm for a 3-component mixture linear regression model
#'
...
#' @export
#'
admm_linear3 <- function(y,x,w,beta_0,alpha1_0,alpha2_0,alpha3_0,vlambda1,vlambda2,eps=1e-6,max=50){
  P <- length(beta)
  ...
  while (convergence>eps & i<max){
    new.theta <- gd_theta3(y,x,w,beta,alpha1,alpha2,alpha3,phi,beta_t,alpha1_t,alpha2_t,alpha3_t)
    new.beta <- new.theta$beta
    new.alpha1 <- new.theta$alpha1
    new.alpha2 <- new.theta$alpha2
    new.alpha3 <- new.theta$alpha3

    new.psi <- glasso3(new.beta,new.alpha1,new.alpha2,new.alpha3,phi,vlambda1,vlambda2)
    new.beta_t <- new.psi$beta_t
    new.alpha1_t <- new.psi$alpha1_t
    new.alpha2_t <- new.psi$alpha2_t
    new.alpha3_t <- new.psi$alpha3_t

    new.phi <- gd_phi3(phi,new.beta,new.alpha1,new.alpha2,new.alpha3,new.beta_t,new.alpha1_t,new.alpha2_t,new.alpha3_t)
